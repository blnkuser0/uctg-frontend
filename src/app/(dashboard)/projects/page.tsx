...
export default function ProjectsPage() {
  ...
  const allProjects = useAllProjects(isSuperAdmin && scope === "all");
...
function EmptyProjects({ isSuperAdmin }: { isSuperAdmin: boolean }) { return <section className="border border-dashed border-border bg-card/60 p-8 sm:p-12"><Layers3 className="size-5 text-primary" /><h2 className="mt-6 text-xl font-semibold tracking-[-.03em]">No project assignment yet.</h2><p className="mt-2 max-w-md text-sm leading-6 text-muted-foreground">{isSuperAdmin ? "Create the first project for a client, then add Umbrella developers to its delivery team." : "A Super Admin will add you to client work when your delivery team is ready."}</p>{isSuperAdmin && <div className="mt-6"><NewProjectDialog trigger={<span><Plus className="size-4" /> Create client project</span>} /></div>}</section>; }
