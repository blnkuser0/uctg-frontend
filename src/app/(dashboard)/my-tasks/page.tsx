    ...
    <div className="catalyst-page max-w-[90rem]">
      ...
      <section className="catalyst-panel overflow-hidden">
        ...
        <div className="grid gap-2 p-3 sm:grid-cols-2 xl:grid-cols-[minmax(15rem,1fr)_repeat(4,minmax(8rem,.42fr))_auto]">
          ...
          <Select value={projectId} items={{ all: "All projects", ...Object.fromEntries(projectList.map((project) => [project._id, `${project.key} · ${project.name}`])) }} onValueChange={(value) => setProjectId(value ?? "all")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All projects</SelectItem>{projectList.map((project) => <SelectItem key={project._id} value={project._id}>{project.key} · {project.name}</SelectItem>)}</SelectContent></Select>
          <Select value={stageId} items={{ all: "All statuses", ...Object.fromEntries(stages.map((stage) => [stage._id, stage.name])) }} onValueChange={(value) => setStageId(value ?? "all")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All statuses</SelectItem>{stages.map((stage) => <SelectItem key={stage._id} value={stage._id}>{stage.name}</SelectItem>)}</SelectContent></Select>
          <Select value={priority} items={{ all: "All priorities", urgent: "Urgent", high: "High", normal: "Normal", low: "Low" }} onValueChange={(value) => setPriority(value ?? "all")}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All priorities</SelectItem><SelectItem value="urgent">Urgent</SelectItem><SelectItem value="high">High</SelectItem><SelectItem value="normal">Normal</SelectItem><SelectItem value="low">Low</SelectItem></SelectContent></Select>
          <Select value={due} items={{ all: "Any deadline", overdue: "Overdue", week: "Due this week", none: "No deadline" }} onValueChange={(value) => setDue((value ?? "all") as DueFilter)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">Any deadline</SelectItem><SelectItem value="overdue">Overdue</SelectItem><SelectItem value="week">Due this week</SelectItem><SelectItem value="none">No deadline</SelectItem></SelectContent></Select>
