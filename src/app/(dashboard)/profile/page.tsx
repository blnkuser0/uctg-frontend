...
export default function ProfilePage() {
  ...
  return <div className="catalyst-page">
    ...
    <IdCardPanel />
    <section className="catalyst-panel"><div className="flex items-center justify-between border-b border-border px-5 py-3"><div className="flex items-center gap-2"><KeyRound className="size-4 text-primary" /><h2 className="text-sm font-semibold">Security</h2></div><span className="text-[11px] text-muted-foreground">Changing your password revokes other refresh sessions.</span></div><form onSubmit={updatePassword} className="grid gap-4 p-5 md:grid-cols-[1fr_1fr_auto] md:items-end"><div className="grid gap-1.5"><Label htmlFor="currentPassword">Current password</Label><PasswordInput id="currentPassword" autoComplete="current-password" value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></div><div className="grid gap-1.5"><Label htmlFor="newPassword">New password</Label><PasswordInput id="newPassword" autoComplete="new-password" value={newPassword} onChange={(event) => setNewPassword(event.target.value)} /></div><Button type="submit" variant="outline" disabled={changePassword.isPending || !currentPassword || newPassword.length < 8}>{changePassword.isPending ? "Updating..." : "Update password"}</Button></form></section>
  </div>;
}
