...
function ResetPasswordForm() {
  ...
  return <AuthFrame eyebrow="Secure reset" title="Set a new password" description="Choose a new password for your Catalyst account. Existing refresh sessions will be revoked.">
    {!token ? <div className="border-l-2 border-destructive bg-card p-4 text-sm text-destructive">This reset link is missing or invalid. Request a new link.</div> : <form className="grid gap-5" onSubmit={handleSubmit(onSubmit)}><div className="grid gap-2"><Label htmlFor="password">New password</Label><PasswordInput id="password" autoComplete="new-password" className="h-11 bg-card" {...register("password")} />{errors.password && <p className="text-xs text-destructive">{errors.password.message}</p>}</div><div className="grid gap-2"><Label htmlFor="confirmPassword">Confirm password</Label><PasswordInput id="confirmPassword" autoComplete="new-password" className="h-11 bg-card" {...register("confirmPassword")} />{errors.confirmPassword && <p className="text-xs text-destructive">{errors.confirmPassword.message}</p>}</div><Button type="submit" disabled={isSubmitting} className="h-11 bg-primary font-mono text-[11px] tracking-[.12em] text-primary-foreground uppercase">{isSubmitting ? "Updating..." : "Update password"}</Button></form>}
